...
| `.claude/rules/worker-state.md` | `src/hook.ts`, `src/hooks.ts`, `src/scheduler.ts`, `src/tools/wakes.ts`: why `agent_state_log` is append-only and how to assert over it, four ways worker state has been wrong, and how each was closed |
