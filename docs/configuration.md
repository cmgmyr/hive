...
| `HIVE_SCHEDULER_INTERVAL_MS` | Testing override for how often the MCP server's scheduler ticks. Default `3000`. A value that is not a whole number of at least `100` is ignored and the default is used |
