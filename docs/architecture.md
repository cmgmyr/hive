...
Two harnesses drive `agent_state` through `src/hook.ts`, and each supplies its own evidence (todo 525). Claude Code fires `Stop`, `UserPromptSubmit`, `Notification`, and `SessionEnd`, mapped to hive's own argv labels (`src/hooks.ts:31-32`, plus `Stop` at :29 and `UserPromptSubmit` at :30): `Stop` to `stop`, `UserPromptSubmit` to `prompt`, `Notification` to `notify`, `SessionEnd` to `session_end`. Codex fires the same `stop`/`prompt` pair plus two more, `SubagentStart`/`SubagentStop` (`src/codexHome.ts`), and carries no `Notification` analog at all - `notify` is claude-only and proven unreachable for codex by construction, not merely unused (`test/codex-notify-unreachable.test.mjs`). `stateFor` (`src/hook.ts:167-191`) turns an event into a state: `prompt` is always `working`; `stop` is `idle` unless a subagent is still live, decided from whichever channel the payload actually carries rather than trying both (`waitingOnSubagents`, `:68-73`) - claude's own `background_tasks` field when the payload carries it at all, or codex's `SubagentStart`/`SubagentStop` log rows read back within a bounded window when it does not (`hasOpenSubagent`, `:84-107`; model, and why a crash, a second concurrent subagent, or log retention evicting the evidence cannot break it: `.claude/skills/hive-internals/references/worker-state.md`). `subagent_start`/`subagent_stop` are log-only and never force a state write, the same as `notify`'s `idle_prompt` case. `notify` becomes `waiting` via `stateForNotification` (`:113-121`), except `idle_prompt`, which returns `null`. An event `stateFor` has no case for does nothing (`null`) rather than asserting a state it does not have - an earlier version returned `waiting` for anything unrecognized, indistinguishable from a real notify payload's own fallback. That does not skip the log: `record()` runs unconditionally with `state ?? UNCHANGED` (`:224`), so a log-only or unmapped event still appends a row to `agent_state_log`, with state `unchanged`. Every hook invocation appends exactly one row that way, keyed by `HIVE_AGENT_ID` (`.claude/rules/worker-state.md`).
...
`HIVE_DATA_DIR` resolves to a `storeDir()` (`src/dataDir.ts:9,47-54`, guarded against the default store for a non-product entry point), and `dataDirTag()` derives a short hash of that directory for anything that has to stay unique per store (`:95-96`). The tmux session name is one of those things: `hive-{tag}main`, one session per store rather than one per project (`.claude/rules/tmux-and-panes.md`). A tmux socket path is computed at spawn or resume time (`tmuxSocketPath`, `src/tmux.ts:291`) and stored per agent row (`agents.tmux_socket`). Every later liveness check compares that recorded socket against the current one (`foreignSocket`, `:553`); when they differ, hive is looking at a row it cannot see into, and the answer is `null`, never a guess (`rowLive`, `:557-559`).
