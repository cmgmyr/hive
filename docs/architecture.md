...
`ensureWorkerHooksFile` (`src/hooks.ts:46`) adds the statusline wrapper to separate Claude worker settings. It registers `PostToolUse` only when the project configures `context_checkpoint_percent`. `ensureCodexHooksFile` (`src/codexHome.ts:270`) applies the same worker-only gate to each generated Codex home. Spawn and resume capture the project value; a running worker keeps its launch value. Common Claude settings and Codex lead homes omit this hook. With no configured threshold, no PostToolUse process starts.
