...
`ci: weekly` is another such var, prose only: hive does not read it or gate anything on it. Absent, it says nothing about a project's CI cadence. Set, it is a project's own record that its CI runs on a schedule plus `workflow_dispatch` rather than on every push and pull request, with a local full suite standing in as the merge gate; a profile's own runbook or posture text is what actually interprets it, the same as `check`.
